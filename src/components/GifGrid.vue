...
<template>
  <div class="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
    <div v-for="(gif, index) in filteredGifs" :key="index" class="rounded-lg cursor-pointer p-4 m-4 hover:scale-110 transition-transform duration-300">
				<img src="../assets/extras/download.svg" alt="donwload" class="bg-gradient-to-r from-pink-500 to-purple-700 text-white rounded-sm w-6 p-1 my-1" @click="downloadImage(gif.images.original.url)">
      <div class="relative">
